...
async def loss_response(callback: types.CallbackQuery, state: FSMContext, qindex: int, username) -> None:
    if callback.message:
        ...
        if qindex > 10:
            ...
            flavorcarryover.append(performance)
        logger.info(f"{username} lost the game on round {qindex} at {dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
async def q_response(callback: types.CallbackQuery, state: FSMContext, username) -> None:
    ...
    if callback.message:
        ...
        if callback.data == answer:
            ...
            if round == 13:
                ...
                logger.info(f"{username} won the game at {dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
async def addquestion(msg: types.Message, state: FSMContext, username) -> None:
    ...
    if admin == "admin":
        ...
        await msg.answer(text="I'd love to help you add a question, yet, for security reasons, only staff have permission to do so. " \
...
async def finishquestion(callback: types.CallbackQuery, state: FSMContext, username) -> None:
    if callback.message:
        await state.update_data(answer=callback.data)
        data = await state.get_data()
        logger.info(f"{username} added a question: \"{data['question']}\" with options {data['option1']}, {data['option2']}, {data['option3']}, {data['option4']} and answer {data['answer']}")
        await db_operations.commit_question(data["question"], [data["option1"], data["option2"], data["option3"], data["option4"]], data["answer"])
        builder = InlineKeyboardBuilder()
